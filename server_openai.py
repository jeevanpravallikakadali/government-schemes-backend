...
# AI eligibility checker using OpenAI directly
async def check_scheme_eligibility(family_data: Dict, scheme_name: str) -> Dict:
    """Use OpenAI to check eligibility for a specific scheme"""
    
    if not openai_client:
        return {
            "eligible": False,
            "reasoning": "AI service not configured. Please set OPENAI_API_KEY environment variable or install OpenAI library.",
    ...
    Analyze eligibility and respond in JSON format:
    {{
        "eligible": boolean,
    ...
    try:
        response = await openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an expert in Indian government schemes eligibility. Respond only with valid JSON."},
        ...
        logging.error(f"OpenAI eligibility check failed: {str(e)}")
        return {
            "eligible": False,
...
# All routes remain the same as original server.py
